...
namespace {
    ...
    // NSerror domain for errors coming from the QCAR application template classes
    NSString * QCAR_APPLICATION_ERROR_DOMAIN = @"QCAR_application";
}

@interface TTQCARSession ()
...
// TTQCARControl delegate (receives callbacks in response to particular
...
@implementation TTQCARSession
@synthesize viewport;

- (id)initWithDelegate:(id<TTQCARControl>) delegate
{
    self = [super init];
    if (self) {
...
- (NSError *) NSErrorWithCode:(int) code {
    return [NSError errorWithDomain:QCAR_APPLICATION_ERROR_DOMAIN code:code userInfo:nil];
}
...
- (void) errorMessage:(NSString *) message {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:QCAR_APPLICATION_ERROR_DOMAIN
                                                    message:message
                                                   delegate:nil
